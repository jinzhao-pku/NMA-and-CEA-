...
filename$trt<-c("che","cam+che","che","niv+che","che","tori+che","che","pemb+che","che","serp+che","che","sinti+che")
filename$study <- c("ESCORT-1st","ESCORT-1st","CheckMate-648","CheckMate-648","Jupiter-06","Jupiter-06","KEYNOTE-590","KEYNOTE-590","ASTRUM-007","ASTRUM-007","ORIENT-15","ORIENT-15")
...
for (i in 1:long) {
  ...
  data_temp$txcode <- case_when(data_temp$trt=="che"~"1",
                                ...
                                data_temp$trt=="serp+che"~"6",
                                data_temp$trt=="sinti+che"~"7",
                                )
  assign(IPDfilename[i,1],data_temp)
  write.table(data_temp,filename[i,1],row.names = FALSE)
}
combined_IPD<-rbind(IPD_cam_pfs0,IPD_cam_pfs1,IPD_nivol_pfs0,IPD_nivol_pfs1,IPD_tori_pfs0,IPD_tori_pfs1,IPD_pemb_pfs0,IPD_pemb_pfs1,IPD_serp_pfs0,IPD_serp_pfs1,IPD_sinti_pfs0,IPD_sinti_pfs1)
